...
function constructIncidents(parentIncident, incidentDefinition) {
  ...
  if (parentIncident.hasIncidents) {
    for (let i = 0; i < incidentDefinition.incidents.length; i++) {
      ...
      const theNewIncident = new incidentItem.incident.Incident(
        incidentItem.incident.attrs,
        incidentItem.incident.props
      );
      ...
      // a special check for a special case. While Group objects maintain their reference
...
function IncidentFromDefinition(definition) {
  ...
  const theNewIncident = new definition.Incident(definition.attrs, definition.props);
...
export default IncidentFromDefinition;
