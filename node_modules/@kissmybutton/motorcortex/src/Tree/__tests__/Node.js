...
import { _LEAFALREADYASSIGNED, _LEAFNOTFOUND, _NEGATIVEPOSITIONNOTALLOWED } from '../_copy';
...
test('should return the failed payload for _UP messages when it does not belong to a tree and selfExecute=true', () => {
  expect(
    plainNode.putMessageOnPipe('foo', {}, 'foo-target', { selfExecute: true, direction: _DIRECTIONS._UP })
  ).toEqual({
    response: false,
    responder: null
  });
...
test('should return an empty payload for _DOWN messages when it does not belong to a tree and selfExecute=true', () => {
  expect(
    plainNode.putMessageOnPipe('foo', {}, 'foo-target', { selfExecute: true, direction: _DIRECTIONS._DOWN })
  ).toEqual([]);
});
...
test('Gather responses from eligible Leafs', () => {
  ...
  const res = node.putMessageOnPipe('gather', {}, {}, { selfExecute: true, direction: _DIRECTIONS._DOWN });
  expect(res).toEqual([
    {
      response: '_leafA',
      responder: _leafA,
...
test('should not get back an answer as the root is not eligible to handle the root request', () => {
  const res = leafA.putMessageOnPipe('rootRequest', {}, _TARGETTYPES._ROOTOFTREE, {
    selfExecute: false,
