...
class ClipChannel extends Channel {
  ...
  checkAddition(incidents) {
    ...
    const exec = function execFunct() {
      ...
      that.incidents = _sortBy(that.incidents, [laneItem => laneItem.millisecond]);
      for (let i = 0; i < incidents.length; i++) {
  ...
  checkDelete(incidentsArray) {
    ...
    const exec = function execFunct() {
      const newIncidents = _filter(that.incidents, incident => incidentIds.indexOf(incident.id) === -1);
      ...
      if (to > from) {
        participatingItems = _filter(
          ...
          laneItem =>
            ...
            (that._incidentById(laneItem.id).duration + laneItem.millisecond >= to && laneItem.millisecond <= to)
        );
      } else {
        participatingItems = _filter(
          this.incidents,
          laneItem =>
            ...
            (laneItem.millisecond + that._incidentById(laneItem.id).duration >= to &&
              laneItem.millisecond + that._incidentById(laneItem.id).duration <= from) ||
            (that._incidentById(laneItem.id).duration + laneItem.millisecond >= from && laneItem.millisecond <= from)
        );
      }
