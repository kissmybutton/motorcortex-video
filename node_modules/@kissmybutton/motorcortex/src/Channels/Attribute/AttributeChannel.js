...
import { getMCIDandAttrOutOfLaneKey } from '../../_coreUtils/Helper';
...
class AttributeChannel extends Channel {
  ...
  checkAddition(incidents, type = 'all-or-nothing') {
    ...
    for (let i = 0; i < incidents.length; i++) {
      ...
      const overlappingAnims = LanesHandlerSandbox.getOverlappingAnims(laneItem, mcid, attribute);
  ...
  checkEdit(incidents, millisecondsDelta) {
    ...
    for (let i = 0; i < incidents.length; i++) {
      ...
      const { mcid } = incidents[i].incident;
      ...
      for (let k = 0; k < lane.length; k++) {
        if (lane[k].id === incidentId) {
          ...
          const testLaneElement = LanesHandlerSandbox.getLaneElementsClone(affectedAnim);
  ...
  slipIntoLaneForwards(lane, laneData, currentMillisecond, millisecond, contextId, forceReset = false) {
    ...
    const that = this;
    let participatingAnims = _filter(
      lane,
      laneItem =>
        // it ends after currentMillisecond and before target millisecond
        ...
        (laneItem.millisecond + that.incidentsById._get(laneItem.id).duration >= currentMillisecond &&
          laneItem.millisecond + that.incidentsById._get(laneItem.id).duration <= millisecond) ||
    ...
    if (participatingAnims.length === 0) {
      ...
      if (!!forceReset && currentMillisecond === 0) {
    ...
    participatingAnims = _sortBy(participatingAnims, [laneItem => laneItem.millisecond]);
    ...
    if (participatingAnim.duration + startMillisecond <= millisecond) {
      ...
      const percentageComplete = (millisecond - startMillisecond) / participatingAnim.duration;
   ...
   * @param {string} contextId - the contextId of the Incidents to slip
   */
  slipToLaneBackwards(lane, laneData, currentMillisecond, millisecond, contextId) {
    // eslint-disable-line no-unused-vars
  ...
  moveTo(from, to, contextId, forceReset = false) {
    ...
    for (let i = 0; i < laneKeys.length; i++) {
      ...
      const laneData = getMCIDandAttrOutOfLaneKey(laneid);
      if (from <= to) {
