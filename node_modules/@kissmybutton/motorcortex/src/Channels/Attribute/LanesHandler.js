import _filter from '../../../lodash-custom-build/modularize/lodash.filter/index';
import _findIndex from '../../../lodash-custom-build/modularize/lodash.findindex/index';
import _sortBy from '../../../lodash-custom-build/modularize/lodash.sortby/index';
import { getLaneKey, getMCIDandAttrOutOfLaneKey } from '../../_coreUtils/Helper';
import Collection from './helpers/Collection';
import setInitialValueForCombo from './helpers/ComboAttrsCascading';
import SandboxCollection from './helpers/SandboxCollection';
import SandboxIncidentsById from './helpers/SandboxIncidentsById';
import SandboxLanes from './helpers/SandboxLanes';
...
class LanesHandler {
  ...
  getOverlappingAnims(newAnim, mcid, attr, excludeIdsFromCheck = [], testDuration = null) {
    const that = this;
    const overlappingAnims = _filter(this.lanes._get(getLaneKey(mcid, attr)), existingAnim => {
      ...
      return (
        ...
        ((existingAnim.millisecond >= newAnim.millisecond &&
          ...
          (existingAnim.millisecond + that.incidentsById._get(existingAnim.id).duration > newAnim.millisecond &&
            existingAnim.millisecond + that.incidentsById._get(existingAnim.id).duration <=
              durationToUseOnChecks + newAnim.millisecond) ||
  ...
  addElementToLane(mcid, attr, millisecond, incident) {
    ...
    // indexing
    const newAnimsIndex = _findIndex(theLane, item => item.id === incident.id);
  ...
  updateLane(affectedAnimationsIds, millisecondsDelta) {
    ...
    for (const laneKey in affectedLanes) {
      ...
      let { lane } = affectedLane;
      const { laneData } = affectedLane;
      ...
      const sortedLane = _sortBy(lane, ['millisecond']);
      this.lanes._set(laneKey, sortedLane);
      lane = sortedLane;

      for (let i = 0; i < affectedLane.animations.length; i++) {
        const animationToCheck = affectedLane.animations[i];
        const animationIndexBeforeEdit = _findIndex(laneBeforeEdit, item => item.id === animationToCheck);

        const animationIndexAfterEdit = _findIndex(lane, item => item.id === animationToCheck);
        ...
        if (animationIndexBeforeEdit !== animationIndexAfterEdit || animationIndexAfterEdit > 1) {
            ...
            if (animationIndexBeforeEdit === 0) {
              ...
              if (isComboAttr) {
                ...
                setInitialValueForCombo(
                  lane,
                  this.incidentsById,
                  this.incidentsById._get(laneBeforeEdit[animationIndexBeforeEdit - 1].id).animatedAttributeValue,
                  indexToAffect,
                  true
                );
              } else {
                this.incidentsById
                  ._get(laneBeforeEdit[animationIndexBeforeEdit + 1].id)
                  .setInitialValue(
                    this.incidentsById._get(laneBeforeEdit[animationIndexBeforeEdit - 1].id).animatedAttributeValue
                  );
          ...
          if (animationIndexAfterEdit === 0) {
            ...
            if (isComboAttr) {
              setInitialValueForCombo(
                lane,
                this.incidentsById,
                this.incidentsById._get(laneBeforeEdit[0].id).pureInitialValues,
                animationIndexAfterEdit,
                true
              );
            } else {
            ...
            if (isComboAttr) {
              setInitialValueForCombo(
                lane,
                this.incidentsById,
                this.incidentsById._get(lane[animationIndexAfterEdit - 1].id).animatedAttributeValue,
                animationIndexAfterEdit,
                true
              );
            } else {
          ...
          if (animationIndexAfterEdit + 1 < lane.length) {
            ...
            if (isComboAttr) {
              ...
              this.incidentsById
                ._get(lane[animationIndexAfterEdit + 1].id)
                .setInitialValue(animation.animatedAttributeValue);
      ...
      for (let j = 0; j < belongingLanes.length; j++) {
        ...
        if (lane.length === 0) {
          ...
          if (
            ...
            this.incidentsById._get(animationToDelete.id).pureInitialValues !== false
          ) {
            if (Object.prototype.hasOwnProperty.call(this.comboAttributes, laneData.attribute)) {
              setInitialValueForCombo(
                lane,
                this.incidentsById,
                this.incidentsById._get(animationToDelete.id).pureInitialValues,
                animationIndexBeforeEdit,
                true
              );
            } else {
              this.incidentsById
                ._get(lane[animationIndexBeforeEdit].id)
                .setInitialValue(this.incidentsById._get(animationToDelete.id).pureInitialValues);
