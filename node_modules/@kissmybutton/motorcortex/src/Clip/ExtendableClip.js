...
class ExtendableClip extends Group {
  ...
  addContext(contextData) {
    ...
    const res = this.putMessageOnPipe(
      'addContext',
      contextData,
      {},
  ...
  checkAddition(incidentsById, type = 'all-or-nothing') {
    ...
    const toReturnExecute = function toReturnExecuteFunct() {
  ...
  checkResize(durationFraction, incidentsById, adjustMillisecond = 0) {
    ...
    for (const key in incidentsById) {
      // perform the check
      const adjustedIncidents = helper.systoleDiastoleProjections(
        incidentsById[key],
        durationFraction,
        adjustMillisecond
      );
  ...
  // eslint-disable-next-line no-unused-vars
  onAfterProgress(fraction, milliseconds) {}
