...
var FullClip = new MC.Clip({
  html: '',
  css: '',
  selector: '#host2',
  id: 'fullClip'
});
var PlaybackHello = new MC.AudioPlayback({}, {
  selector: '~#hello',
  duration: 6000,
  startFrom: 1200,
  id: 'playback-hello'
});
test("expect AudioClips to contain only audioClip and no real Tree", function () {
  expect(AudioClip.realClip).toBeInstanceOf(NullClip);
  expect(AudioClip2.realClip).toBeInstanceOf(NullClip);
});
test("expect a DescriptiveClip with audio false not to have an audioClip", function () {
  expect(MutedClip.audioClip).toBeInstanceOf(NullClip);
});
test("expect a DescriptiveClip with audio on to have an audioClip", function () {
  expect(FullClip.audioClip).toBeInstanceOf(ReadAudioClip);
});
test("Add AudioClip to another AudioClip", function () {
  var res = AudioClip.addIncident(AudioClip2, 0);
  expect(res.result).toBe(true);
  expect(AudioClip.audioClip.instantiatedChannels['@kissmybutton/self-contained-incidents'].incidents.length).toBe(1);
  expect(AudioClip.audioClip.instantiatedChannels['@kissmybutton/self-contained-incidents'].incidents).toEqual([{
    id: 'audioClip2',
    millisecond: 0
  }]);
  expect(AudioClip2.audioClip.context.masterNode.output.connection).toBe(AudioClip.audioClip.context.masterNode.input);
});
test("move an AudioClip", function () {
  var res = AudioClip.moveIncident(AudioClip2, 1000);
  expect(res.result).toBe(true);
  expect(AudioClip.audioClip.instantiatedChannels['@kissmybutton/self-contained-incidents'].incidents).toEqual([{
    id: 'audioClip2',
    millisecond: 1000
...
test("resize an AudioClip that has an empty AudioClip in it", function () {
  var res = AudioClip.resize(2000);
  expect(res.result).toBe(true);
});
test('remove an AudioClip from another AudioClip and expect to get disconnected from its node', function () {
  var res = AudioClip.removeIncident(AudioClip2);
  expect(res.result).toBe(true);
  expect(AudioClip.audioClip.instantiatedChannels['@kissmybutton/self-contained-incidents'].incidents.length).toBe(0);
  expect(AudioClip2.audioClip.context.masterNode.output.connection).toBe('destination');
  expect(AudioClip.duration).toBe(0);
});
test("adding a muted clip to an AudioClip should result to no change", function () {
  var res = MutedClip.addIncident(AudioClip, 0);
  expect(res.result).toBe(true);
  expect(MutedClip.realClip.instantiatedChannels['@kissmybutton/self-contained-incidents']).toBe(undefined);
});
test("adding an AudioPlayback by id selector should add one MediaPlayback Incident to channels", function () {
  AudioClip.addIncident(AudioClip2, 0);
  var res = AudioClip2.addIncident(PlaybackHello, 1000);
  expect(res.result).toBe(true);
  expect(AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback']).toBeInstanceOf(AudioPlaybackChannel);
  expect(AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback'].incidents).toEqual([{
    id: 'playback-hello_hello',
    millisecond: 1000
  }]);
  expect(AudioClip2.duration).toBe(7000);
  expect(MutedClip.duration).toBe(7000); // console.log(AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback'].incidents);
});
test("moving an AudioPlayback should reflect properly in channels", function () {
  var res = AudioClip2.moveIncident(PlaybackHello, 2000);
  expect(res.result).toBe(true);
  expect(AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback']).toBeInstanceOf(AudioPlaybackChannel);
  expect(AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback'].incidents).toEqual([{
    id: 'playback-hello_hello',
  ...
  expect(MutedClip.duration).toBe(8000); // console.log(AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback'].incidents);
});
test("test execution (start / stop) of AudioPlayback Incident", function () {
  AudioClip.play(true);
  ...
  AudioClip.playableProgress(1800 / AudioClip.duration, 1800);
  expect(AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback'].incidentsById['playback-hello_hello'].contexts['audioClip2'].audioNode.exportLog()).toEqual([{
    event: 'start',
    ms: 1.300
  }, {
    event: 'stop'
  }]); // console.log(AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback'].incidentsById['playback-hello_hello'].contexts['audioClip2'].element.audioNode.exportLog());
  // console.log(AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback'].incidentsById['playback-hello_hello'].exportLog());
});
test("make sure that AudioPlaybacks are not executed on seek", function () {
  AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback'].incidentsById['playback-hello_hello'].contexts['audioClip2'].audioNode.clearLog();
  var journey = TimeCapsule.startJourney(AudioClip);
...
test("expect the currentMillisecond of audioClip to be up to date even though it doesn't participate on seek", function () {
  expect(AudioClip2.audioClip.runTimeInfo.currentMillisecond).toBe(1800);
});
test("expect the audioClip and audio playback to execute properly after journey", function () {
  AudioClip.play(true);
  AudioClip.playableProgress(2100 / AudioClip.duration, 2100);
  expect(AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback'].incidentsById['playback-hello_hello'].contexts['audioClip2'].audioNode.exportLog()).toEqual([{
    event: 'start',
  ...
  expect(AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback'].playingIncidentsIds).toEqual(['playback-hello_hello|||audioClip2']);
});
test("expect AudioPlayback to stop when seek starts", function () {
  expect(AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback'].incidentsById['playback-hello_hello'].contexts['audioClip2'].audioNode.status).toEqual('playing');
  var journey = TimeCapsule.startJourney(AudioClip);
  journey.station(1800);
  journey.station(2100);
  expect(AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback'].incidentsById['playback-hello_hello'].contexts['audioClip2'].audioNode.status).toEqual('stopped');
  expect(AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback'].playingIncidentsIds).toEqual([]);
  journey.destination(1800);
});
test("expect all playing Playbacks to stop on pause", function () {
  AudioClip2.audioClip.instantiatedChannels['@kissmybutton/media-playback'].incidentsById['playback-hello_hello'].contexts['audioClip2'].audioNode.clearLog();
  AudioClip.play(true);
  AudioClip.playableProgress(2100 / AudioClip.duration, 2100);
  AudioClip.pause();
