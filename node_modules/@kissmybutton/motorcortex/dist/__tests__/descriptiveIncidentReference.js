...
var myClip = new DescriptiveClip({
...
test('check that realClip and audioClip of a DescriptiveClip have the right reference to their DescriptiveIncident', function () {
  expect(myClip.realClip.DescriptiveIncident).toBe(myClip);
  expect(myClip.audioClip.DescriptiveIncident).toBe(myClip);
});
test('check that an audioClip entering another audioClip has the right reference to its DescriptiveIncident', function () {
  myClip.addIncident(AudioClip, 0);
  expect(AudioClip.audioClip.DescriptiveIncident).toBe(AudioClip);
});
test('check that all MonoIncidents of an Incident have the right reference to their DescriptiveIncident', function () {
  document.body.innerHTML = "<div id=\"clip-container\"></div>\n    ";
  myClip.addIncident(di1, 1000);
  expect(myClip.realClip.children['di1'].leaf.children['di1_container-1'].leaf.children['di1_container-1_alpha'].leaf.contexts['myClip'].DescriptiveIncident).toBe(di1);
  expect(myClip.realClip.children['di1'].leaf.children['di1_container-2'].leaf.children['di1_container-2_alpha'].leaf.contexts['myClip'].DescriptiveIncident).toBe(di1);
  var host = document.getElementById('clip-container');
  var myRootClip = new DescriptiveClip({
  ...
  expect(Object.keys(myClip.realClip.children['di1'].leaf.children['di1_container-1'].leaf.children['di1_container-1_alpha'].leaf.contexts).length).toBe(3);

  for (var key in myClip.realClip.children['di1'].leaf.children['di1_container-1'].leaf.children['di1_container-1_alpha'].leaf.contexts) {
    var instance = myClip.realClip.children['di1'].leaf.children['di1_container-1'].leaf.children['di1_container-1_alpha'].leaf.contexts[key];
    expect(instance.DescriptiveIncident).toBe(di1);
  }
});
test('check that all AudioPlayback Incidents have the right reference to their DescriptiveIncident', function () {
  AudioClip.addIncident(PlaybackHello, 0);
  expect(myClip.audioClip.children['audioClip2'].leaf.children['playback-hello'].leaf.children['playback-hello_hello'].leaf.DescriptiveIncident).toBe(PlaybackHello);
});
test('check that a group has the right reference to its DescriptiveIncident', function () {
  var myGroup = new MC.Group({
    id: 'myGroup'
  });
  myClip.addIncident(myGroup, 0);
  expect(myClip.realClip.children['myGroup'].leaf.DescriptiveIncident).toBe(myGroup);
});